...
export default function Simulator() {
  ...
  const channels = useMemo<CampaignInput[]>(() =>
    BASE_CHANNELS.map(c => ({
      ...c,
      maxSpendDaily: (caps[c.id] ?? activeCampaigns.find(a => a.id === c.id)?.proposedMonthly ?? 0) / 30,
    })),
    [caps]
  );
    ...
    <section id="simulator">
      ...
      <div className="card" style={{ marginBottom: 24, padding: "20px 24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
          ...
          <span style={{ fontSize: 24, fontWeight: 700, color: "var(--text-0)", fontFamily: "var(--mono)" }}>
            {fmt.usd(budget)}
          </span>
        </div>
        <input
          type="range" min={60000} max={200000} step={1000}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12, marginBottom: 32 }}>
        {[
          { label: "Rev. estimada / mes", value: fmt.usd(result.totalMonthlyRev), cls: "green" },
          { label: "ROAS blended",        value: fmt.pct(result.blendedRoas),     cls: ""      },
          { label: "Rev. estimada / día", value: fmt.usd(result.totalDailyRev),   cls: "green" },
        ].map(k => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "300px 1fr", gap: 32, alignItems: "start" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ fontSize: 11, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--text-2)", marginBottom: 4 }}>
            Cap por campaña
          </div>
            ...
            return (
              <div key={c.id} className="slider-row">
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, marginBottom: 6 }}>
                  <span style={{ fontFamily: "var(--mono)", fontSize: 10, color: "var(--text-2)" }}>{c.id}</span>
                  <span style={{ fontFamily: "var(--mono)", fontWeight: 700, color: "var(--text-0)" }}>{fmt.usd(capVal)}</span>
                </div>
                <input
                ...
                {a && (
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, marginTop: 4 }}>
                    <span style={{ color: "var(--text-2)" }}>Óptimo: {fmt.usd(a.monthlySpend)}</span>
                    <span style={{ color: a.roas >= 0.35 ? "var(--green)" : "var(--amber)" }}>
                      {fmt.pct(a.roas)} · mROAS {a.mroas.toFixed(2)}
                    </span>
                  </div>
                )}
              </div>
          ...
          <div style={{ width: "100%", height: 320, background: "var(--bg-2)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: "20px 12px 12px" }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={sweep} margin={{ top: 4, right: 28, bottom: 4, left: 8 }}>
                ...
                <ReferenceLine
                  yAxisId="rev"
                  x={Math.round(budget / 1000)}
                  stroke="var(--purple)"
                  strokeDasharray="4 2"
                  label={{ value: "actual", fill: "var(--purple-light)", fontSize: 10 }}
                />
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
              <thead>
                <tr style={{ background: "var(--bg-3)", borderBottom: "1px solid var(--border)" }}>
                  <th style={{ padding: "10px 14px", textAlign: "left", fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--text-2)" }}>Campaña</th>
                  <th style={{ padding: "10px 14px", textAlign: "right", fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--text-2)" }}>Óptimo/mo</th>
                  <th style={{ padding: "10px 14px", textAlign: "right", fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--text-2)" }}>ROAS</th>
                  <th style={{ padding: "10px 14px", textAlign: "right", fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "var(--text-2)" }}>mROAS</th>
                </tr>
              </thead>
              <tbody>
                {result.allocations.sort((a, b) => b.monthlySpend - a.monthlySpend).map(a => (
                  <tr key={a.campaignId} style={{ borderBottom: "1px solid var(--border)" }}>
                    <td style={{ padding: "8px 14px", fontFamily: "var(--mono)", fontSize: 10, color: "var(--text-2)" }}>{a.campaignId}</td>
                    <td style={{ padding: "8px 14px", textAlign: "right", fontWeight: 600, color: "var(--text-0)" }}>{fmt.usd(a.monthlySpend)}</td>
                    <td style={{ padding: "8px 14px", textAlign: "right", color: a.roas >= 0.35 ? "var(--green)" : a.roas >= 0.20 ? "var(--amber)" : "var(--red)" }}>{fmt.pct(a.roas)}</td>
                    <td style={{ padding: "8px 14px", textAlign: "right", color: "var(--text-2)", fontFamily: "var(--mono)" }}>{a.mroas.toFixed(3)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
